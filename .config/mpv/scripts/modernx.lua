...
local user_opts = {
  ...
  title_font_size = 32, -- font size of the title text (above seekbar)
  ...
  download_path = "~~desktop/", -- default download directory for videos (https://mpv.io/manual/master/#paths)
...
local osc_styles = {
  ...
  box_bg = "{\\blur"
    ...
    .. "&}",
  title_bar_box_bg = "{\\blur"
    .. user_opts.title_bar_fade_blur_strength
    .. "\\bord"
    .. user_opts.title_bar_fade_alpha
    .. "\\1c&H000000&\\3c&H"
    .. osc_color_convert(user_opts.osc_color)
    .. "&}",
...
function check_comments()
  ...
  mp.command_native_async({
    ...
    if mp.get_property "filename" then
      ...
      filename = mp.command_native { "expand-path", user_opts.comments_download_path .. "/" }
...
layouts["original"] = function()
  ...
  if
    not user_opts.title_bar_box
    and (user_opts.window_top_bar == "yes" or not state.border or not state.title_bar or state.fullscreen)
    and top_titlebar
  then
  ...
  geo = {
    x = 25,
    y = refY - 117 + (((state.localDescription ~= nil or state.is_URL) and user_opts.show_description) and -20 or 0),
    an = 1,
    w = osc_geo.w - 50,
    h = 35,
  }
...
if user_opts.key_bindings then
  ...
  -- chapter scrubbing
  mp.add_key_binding("[", "prevfile", function()
    mp.commandv("playlist-prev", "weak")
    destroyscrollingkeys()
  end)
  mp.add_key_binding("]", "nextfile", function()
    mp.commandv("playlist-next", "weak")
    destroyscrollingkeys()
  end)
  mp.add_key_binding("shift+up", "prevchapter", function()
    change_chapter(-1)
  end)
  mp.add_key_binding("shift+down", "nextchapter", function()
    change_chapter(1)
  end)
  ...
  if user_opts.persistent_progress_toggle then
    mp.add_key_binding("Tab", "persistenttoggle", function()
      if user_opts.persistent_progress_toggle then
  ...
  mp.add_key_binding(nil, "get_chapterlist", function()
    show_message(get_chapterlist())
  end)
